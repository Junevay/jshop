 ...
 *
 */
function buildSelect($tableName, $selectName, $valueFieldName, $textFieldName, $selectedValue = '',$filter='')
{
	$model = D($tableName);
	$data = $model->field("$valueFieldName,$textFieldName")->select();
	$select = "<select lay-filter='$filter' name='$selectName'><option value=' '>请选择</option>";
	foreach ($data as $k => $v)
	{
		...
		else
		{
		    ...
		    if($thumb)
		    {
		    	...
		    	foreach ($thumb as $k => $v)
		    	{
				    ...
				    $width= $image->width();
				    $hight=$image->height();
				    $width=$width/$hight*$v[0];
				    $image->thumb($width, $v[1])->save($ic['rootPath'].$ret['images'][$k+1]);
		    	}
		    }
		    return $ret;
		}
